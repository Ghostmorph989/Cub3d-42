...
/*   Updated: 2019/12/03 09:16:25 by malaoui          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_display_content(t_lib *foo)
{
	...
	if (!(foo->mlx_ptr = mlx_init()))
		return ;
	foo->win_ptr = mlx_new_window (foo->mlx_ptr, foo->display_x, foo->display_y, "Cub3d --Project-- ");
	// foo->display_x /= foo->map_index;
	// foo->display_y /= ft_strlen(*(foo->map));
